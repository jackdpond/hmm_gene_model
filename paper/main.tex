...
to determine that the period of budding yeast's metabolic cycle is 5 hours, and we distinguish the phases of the cycle. We also investigate which genes drive each phase and which biological processes those genes represent.
\end{abstract}
...
Our dataset consists of a sequence of yeast microarray assays. Each microarray was designed to measure the abundance of the same set of 6,400 yeast gene transcripts along with other RNA transcripts of interest, bringing the total to 9725 transcriptomic features per assay. These assays were performed separately on 36 samples taken at regular 25-minute intervals from a metabolically oscillating yeast population. Our dataset is publicly available at the Gene Expression Omnibus \cite{GEO2005} and comes from a peer-reviewed publication in \textit{Science} \cite{Tu2005}.
...
Our analyses can essentially be split into two types--clustering and time series forecasting. For clustering analyses, we use the entire dataset, except to ascertain optimal parameters. For forecasting analyses, we split the dataset according to the period discovered in the clustering analyses, which is 12 timesteps. For all splits, we use the first two periods (24 time steps) for training and the third period (the last 12 time steps) for the test set.
...
Predictions over the full 36-time-point sequence with $k=5$ show a repeating cluster sequence, directly recovering the oscillation period. Interestingly enough, even with an observation space of just 5 dimensions and a hidden state space of just 5, the assignments are still very high probability, indicating high separability.

Additionally, as a sanity check, we fit a GMM on the 200 highest variance genes and then projected to 2 principal components, and graphed the 36 time steps colored by their GMM assignment. There are 5 distinct clusters in 2 dimensions which are correctly discovered by the GMM in high dimensions. Plotting a line connecting the time-points in order reveals the looping, periodic nature of the data.

\textbf{Approach 3. Continuous Density Hidden Markov Model.} We also attempt to discover hidden states using a Continuous Density Hidden Markov Model (CDHMM) approach. We using PCA on our \textit{arcsin} normalized dataset to capture the directions of highest variance, and then use
...
\begin{figure}
    ...
    \caption{The hidden states predicted by a CDHMM approach over the course of the metabolic cycle.}
\end{figure}
...
Note that here we cluster \textit{genes}, whereas earlier we clustered \textit{time steps}. The time-step clustering recovered period-length, and the gene clustering recovered distinct temporal modules of co-expressed genes.
