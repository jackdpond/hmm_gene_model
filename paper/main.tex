...
\begin{figure}
    ...
    Figure 1. Temporal cycles in cell state. [TODO: Cleanup and label, maybe plot cluster assignment over time] Left: The 200 genes with the highest variance in the arcsinh transformed data were extracted and clustered with a GMM with four components, then the data dimensionality was reduced via PCA. Right: The same plot with a line connecting adjacent timesteps. When visualized, a clearly periodic pattern was clear in cluster assignment, repeating every 12 timesteps.
\end{figure}

\subsection{Periodic Gene Identification}
We performed our permutation testing to identify the genes with significant 12-timestep delay autocorrelations, in line with our conclusion that a 12 timestep period was most plausible. Via this analysis, 5243 of the genes - over half of all genes - were identified as periodic with the 12 timestep period.
...
\begin{figure}
    ...
    Figure 2: Temporal expression modules. [TODO: Cleanup + Label Graph] Left: UMAP of genes according to the seasonal component of their standardized expressions, colored by GMM cluster. Right: The seven clearly consistent GMM clusters. The noisy cluster (1) was removed from the analysis.
\end{figure}
